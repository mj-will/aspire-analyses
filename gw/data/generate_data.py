...
def precessing_to_aligned(params):
    """Convert precessing parameters to aligned parameters.

    Compute the aligned spin parameters from the precessing parameters
    and removes the precessing parameters
    """
    params["chi_1"] = params["a_1"] * np.cos(params["tilt_1"])
    params["chi_2"] = params["a_2"] * np.cos(params["tilt_2"])

    for p in ["a_1", "a_2", "theta_1", "theta_2", "phi_12", "phi_jl"]:
        params.pop(p, None)
    return params
...
def load_injections(args):
    ...
    if os.path.isfile(args.injection_file):
        print(f"Loading injections from {args.injection_file}")
